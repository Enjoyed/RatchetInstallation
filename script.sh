...
if [[ $? -eq 0 ]]; then
        printf "TENS INTERNET!"
else
        echo "Necessites connexió a internet per executar aquest script"
        exit 1
fi

function _spinner() {
# FUNCTION CREATED BY TASOS LATSAS
# GITHUB: github.com/tlatsas/bash-spinner
    ...
    case $1 in
        ...
        stop)
            ...
            kill $3 &>> /var/log/superscript.log 2>&1
            echo -en "\b["
            if [[ $2 -eq 0 ]]; then
...
touch /var/log/superscript.log
echo "Inicialitzant SUPERSCRIPT (domotica project) per Gerard Fleque"
echo "Log -> /var/log/superscript.log"

sleep 1

start_spinner "Updating your system..."
echo "Executant apt-get update..." >> /var/log/superscript.log
apt-get update &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner "Installing apache2..."
echo "Executant apt-get install apache2..." >> /var/log/superscript.log
apt-get install -y apache2 apache2-utils &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner "Installing PHP..."
echo "Executant apt-get install php..." >> /var/log/superscript.log
apt-get install -y php5 php5-json php5-dev unzip &>> /var/log/superscript.log
stop_spinner $?
printf "\n"
start_spinner "Creating folders..."
echo "Creating folders..." >> /var/log/superscript.log
...
cd /odt/ratchet
echo "Downloading composer..." >> /var/log/superscript.log
php -r "copy('https://getcomposer.org/installer', 'composer-setup.php');" &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Verifying download...'
echo "Verifying download..." >> /var/log/superscript.log
php -r "if (hash_file('SHA384', 'composer-setup.php') === '669656bab3166a7aff8a7506b8cb2d1c292f042046c5a994c43155c0be6190fa0355160742ab2e1c88d40d5be660b410') { echo 'Installer verified'; } else { echo 'Installer corrupt'; unlink('composer-setup.php'); } echo PHP_EOL;" &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Running composer installer...'
echo "Running composer installer..." >> /var/log/superscript.log
php composer-setup.php &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Removing composer installer...'
echo "Removing composer installer..." >> /var/log/superscript.log
php -r "unlink('composer-setup.php');" &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Installing Ratchet...'
echo "Installing Ratchet..." >> /var/log/superscript.log
php composer.phar require cboden/ratchet &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Creating namespace websocketgerard...'
echo "Creating namespace websocketgerard..." >> /var/log/superscript.log
...
echo "Creating composer.json..." >> /var/log/superscript.log
cd /odt/ratchet
...
echo "Updating composer..." >> /var/log/superscript.log
cd /odt/ratchet
php composer.phar update &>> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Creating main server file...'
echo "Creating main server file..." >> /var/log/superscript.log
...
echo "Downloading bootstrap..." >> /var/log/superscript.log
cd /var/www/html/assets/css
wget https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css &>> /var/log/superscript.log
cd /var/www/html/assets/js
wget https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js &>> /var/log/superscript.log
...
cd /var/www/html/assets/js
wget https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js &>> /var/log/superscript.log
echo "Downloading jQuery..." >> /var/log/superscript.log
stop_spinner $?

printf "\n"
start_spinner 'Downloading and installing FontAwesome...'
echo "Downloading FontAwesome..." >> /var/log/superscript.log
cd ~/
wget fontawesome.io/assets/font-awesome-4.7.0.zip &>> /var/log/superscript.log
unzip font-awesome-4.7.0.zip &>> /var/log/superscript.log
...
echo "Making run on startup..." >> /var/log/superscript.log
echo "php /odt/ratchet/bin/server.php" >> /etc/rc.local
stop_spinner $?
sleep 1
printf "\n"
start_spinner 'Setting the default template...'
echo "Setting the default template..." >> /var/log/superscript.log
...
echo "Setting the web updaters..." >> /var/log/superscript.log
...
echo "Modifying permissions..." >> /var/log/superscript.log
